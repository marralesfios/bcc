...
function highlight_declarator(field) {
...
function highlight_struct_or_union(el, sn, type) {
...
function rfn_line(name, custom) {
    ...
    if (custom)
        ...
        dl.append(") = ", keyword("sequential"), ";");
    return dl;
}
function wfn_line(name, custom) {
    const dl = document.createElement("span");
    dl.classList.add("sspec-fn-decl-header");
...
function do_highlight_type(type, to) {
    switch (type.tagName) {
        case "KEY-WORD":
...
function do_highlight_expr(expr, to) {
    switch (expr.tagName) {
        case "CODE-COMMENT":
...
function while_line(cond, dowhile) {
    const ln = document.createElement("span");
    if (dowhile)
        ln.append("}");
...
function highlight_stmt(stmt) {
    const el = document.createElement("span");
    el.classList.add("sspec-stmt");
    do_highlight_stmt(stmt, el);
    return el;
}
function highlight_fn(el, to) {
...
function highlight_readfn(el, sn) {
...
function highlight_writefn(el, sn) {
    const fd = document.createElement("span");
    const custom = el.hasChildNodes();
...
function enum_decl_line(sn, underlying) {
    const dl = document.createElement("span");
    dl.classList.add("sspec-decl-header");
    dl.append(keyword("enum"), " ", sn);
    if (underlying !== null)
        dl.append(" : ", underlying);
    dl.append("{");
    return dl;
}
function highlight_enum(el, sn) {
    const def = document.createElement("span");
    def.classList.add("sspec-class");
...
for (const el of document.getElementsByClassName("sspec")) {
